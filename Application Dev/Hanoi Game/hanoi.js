...
class Rod {
	...
	canPutDisk(disk) {
		return this.isEmpty() || this.topDisk().biggerThan(disk)
	}
	pop() {
		if (this.isEmpty()) {
...
const rod = {
	...
	template: `
		<div 
			...
			@drop="drop($event, rod)"> 
			<div 
				v-for="disk in rod.disks" 
				:key="disk.size" 
				class="disk" 
				:style="{ height: disksHeight(1), width: diskWidth(disk.size), backgroundColor: disk.color }"
				draggable="true"
